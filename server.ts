...
// api/dokumentasi/pdf
app.get("/api/dokumentasi/pdf", (req, res) => {
  try {
    const doc = new PDFDocument({ margin: 50, size: "A4" });

    // Set response headers to stream PDF to user
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", "attachment; filename=DOKUMENTASI_TEKNIS_SIPINJAM_UPT.pdf");

    doc.pipe(res);

    // Color definitions
    const primaryColor = "#312e81"; // Deep Indigo
    const secondaryColor = "#4f46e5"; // Indigo
    const textColor = "#1f2937"; // Dark Gray
    const lightBg = "#f9fafb"; // Light Gray
    const borderColor = "#e5e7eb"; // Silver Gray

    // Page Helper functions
    const addHeader = (sectionTitle: string) => {
      doc.save();
      doc.fontSize(8).fillColor("#9ca3af").font("Helvetica").text("SIPINJAM UPT - DOKUMENTASI TEKNIS", 50, 30, { align: "left" });
      doc.fontSize(8).fillColor("#9ca3af").font("Helvetica").text(sectionTitle, 50, 30, { align: "right" });
      doc.moveTo(50, 42).lineTo(545, 42).strokeColor(borderColor).lineWidth(1).stroke();
      doc.restore();
    };

    const addFooter = (pageNum: number) => {
      doc.save();
      doc.moveTo(50, 790).lineTo(545, 790).strokeColor(borderColor).lineWidth(1).stroke();
      doc.fontSize(8).fillColor("#9ca3af").font("Helvetica").text(`Dibuat secara otomatis oleh Sistem Portal SiPinjam \u2022 Halaman ${pageNum}`, 50, 798, { align: "center" });
      doc.restore();
    };
    ...
    // --- COVER PAGE ---
    // Background decor
    doc.rect(0, 0, 595, 842).fill("#faf9f6");
    doc.rect(0, 0, 15, 842).fill(primaryColor);

    // Main Title
    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(28).text("DOKUMENTASI TEKNIS", 60, 180);
    doc.fillColor(secondaryColor).font("Helvetica-Bold").fontSize(20).text("BAHASA PEMROGRAMAN & TEKNOLOGI", 60, 215);
    
    // Subtitle Decor Line
    doc.moveTo(60, 250).lineTo(300, 250).strokeColor(secondaryColor).lineWidth(4).stroke();

    // App Information
    doc.fillColor(textColor).font("Helvetica").fontSize(13).text("Sistem Informasi Peminjaman Sarana & Prasarana UPT", 60, 275);
    doc.fillColor("#4b5563").font("Helvetica").fontSize(11).text("Sebuah platform digital komprehensif, cepat, transparan, dan akuntabel berbasis teknologi modern Node.js, Express, dan TypeScript.", 60, 295, { width: 450, lineGap: 4 });

    // Details Box
    doc.rect(60, 420, 450, 140).fill("#ffffff").stroke(borderColor);
    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(12).text("INFORMASI DOKUMEN", 80, 440);
    doc.moveTo(80, 455).lineTo(490, 455).strokeColor("#f3f4f6").lineWidth(1).stroke();

    doc.fillColor(textColor).font("Helvetica-Bold").fontSize(9).text("Format Dokumen:", 80, 470);
    doc.font("Helvetica").text("PDF Elektronik (Portable Document Format)", 200, 470);

    doc.font("Helvetica-Bold").text("Bahasa Pemrograman Utama:", 80, 490);
    doc.font("Helvetica").text("TypeScript, JavaScript (Node.js)", 200, 490);

    doc.font("Helvetica-Bold").text("Teknologi Front-End:", 80, 510);
    doc.font("Helvetica").text("HTML5, CSS3, Tailwind CSS, Lucide Icons", 200, 510);

    doc.font("Helvetica-Bold").text("Teknologi Back-End:", 80, 530);
    doc.font("Helvetica").text("Node.js, Express, Multer, NodeMailer", 200, 530);

    // Generation timestamp
    const nowStr = new Date().toLocaleDateString("id-ID", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
    doc.fillColor("#9ca3af").font("Helvetica-Oblique").fontSize(9).text(`Dokumen ini digenerate pada: ${nowStr}`, 60, 750);

    // --- PAGE 2: PENGANTAR & STRUKTUR ---
    doc.addPage();
    currentPage++;
    addHeader("1. PENGANTAR & ARSITEKTUR");
    addFooter(currentPage);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(18).text("1. Pengantar & Arsitektur Sistem", 50, 70);
    doc.moveDown(1);

    doc.fillColor(textColor).font("Helvetica").fontSize(10).text(
      "Web aplikasi SiPinjam UPT dirancang dengan arsitektur modern yang membagi tanggung jawab secara jelas antara Client-Side (Front-End) dan Server-Side (Back-End) menggunakan format full-stack.",
      { align: "justify", lineGap: 3 }
    );
    doc.moveDown(1);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(12).text("Arsitektur Full-Stack (Express + Vite):");
    doc.moveDown(0.5);
    doc.fillColor(textColor).text(
      "1. Server-Side (Back-End / API):\n" +
      "   Menggunakan Node.js dengan framework Express. Server ini bertindak sebagai API gateway, penyimpan data persisten (db_state.json), pengolah otentikasi user, pengelolaan status peminjaman, serta pengunggahan file fisik (foto barang dan surat rekomendasi).\n\n" +
      "2. Client-Side (Front-End / UI):\n" +
      "   Menggunakan struktur HTML5 murni yang diperkaya oleh Tailwind CSS untuk styling responsif, serta ikon modern dari Lucide (via Tabler Icons CDN). Halaman dikemas secara dinamis dengan integrasi JavaScript ES6 untuk berkomunikasi dengan API backend secara asinkron (menggunakan Fetch API).",
      { lineGap: 3 }
    );
    doc.moveDown(1.5);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(12).text("Struktur Folder Proyek:");
    doc.moveDown(0.5);

    // Draw box for directory structure
    doc.save();
    doc.rect(50, doc.y, 495, 130).fill("#f1f5f9").stroke("#cbd5e1");
    doc.fillColor("#0f172a").font("Courier").fontSize(8.5);
    doc.text(
      "├── server.ts             # Entry point Server Express (TypeScript)\n" +
      "├── landing.html          # Halaman Utama (Beranda / Landing Page)\n" +
      "├── login.html            # Halaman Masuk User\n" +
      "├── register.html         # Halaman Pendaftaran Akun\n" +
      "├── admin/                # Panel Pengelola (Dashboard, Kelola Barang, Approval)\n" +
      "├── user/                 # Panel Mahasiswa/Dosen (Katalog, Form Pinjam, Riwayat)\n" +
      "├── js/helpers.js         # Library utilitas klien (otentikasi, render UI, global back)\n" +
      "└── uploads/              # Penyimpanan media (surat/ & barang/)\n",
      65, doc.y + 10, { lineGap: 2.5 }
    );
    doc.restore();

    // --- PAGE 3: TYPESCRIPT & FORMAT FILE ---
    doc.addPage();
    currentPage++;
    addHeader("2. TYPESCRIPT (.TS & .TSX)");
    addFooter(currentPage);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(18).text("2. Mengenal TypeScript (.ts dan .tsx)", 50, 70);
    doc.moveDown(1);

    doc.fillColor(textColor).font("Helvetica").fontSize(10).text(
      "TypeScript adalah bahasa pemrograman open-source yang dikembangkan oleh Microsoft. Bahasa ini merupakan superset dari JavaScript (artinya, setiap kode JavaScript adalah kode TypeScript yang valid) dengan penambahan fitur Static Typing (tipe data statis).",
      { align: "justify", lineGap: 3 }
    );
    doc.moveDown(1);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(12).text("Mengapa Menggunakan TypeScript?");
    doc.moveDown(0.5);
    doc.fillColor(textColor).font("Helvetica").fontSize(10).text(
      "\u2022 Keamanan Kode (Type Safety): Menemukan kesalahan pengetikan atau ketidakcocokan tipe variabel saat compile-time (saat menulis kode), bukan runtime (saat aplikasi dijalankan).\n" +
      "\u2022 Autocomplete & Perkakas: Memberikan petunjuk otomatis (intellisense) yang sangat akurat pada text editor.\n" +
      "\u2022 Struktur Data Jelas: Membantu mendefinisikan bentuk data (Interface/Type) yang disimpan di database atau ditransfer lewat API.",
      { lineGap: 3 }
    );
    doc.moveDown(1.5);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(12).text("Perbedaan Format File .ts dan .tsx:");
    doc.moveDown(0.5);

    // Columns or detailed list
    doc.fillColor(textColor).font("Helvetica-Bold").fontSize(10).text("Format File .ts (TypeScript Murni)");
    doc.font("Helvetica").fontSize(10).text(
      "Format ini digunakan untuk menulis kode TypeScript standar yang hanya berisi logika logika komputasi, algoritma, pemodelan data, routing server, atau fungsi utilitas tanpa ada tampilan visual XML/HTML di dalamnya.\n" +
      "Contoh di proyek ini: server.ts (mengatur konfigurasi server Express, middleware, routing database, penanganan upload surat dan foto).",
      { lineGap: 2 }
    );
    doc.moveDown(1);

    doc.fillColor(textColor).font("Helvetica-Bold").text("Format File .tsx (TypeScript XML)");
    doc.font("Helvetica").text(
      "Format ini khusus digunakan di ekosistem React. Huruf 'x' merujuk pada JSX (JavaScript XML), yang memungkinkan pengembang untuk menulis struktur antarmuka UI mirip HTML langsung di dalam file TypeScript.\n" +
      "Contoh di proyek ini: src/App.tsx, src/main.tsx (digunakan untuk memproses rendering UI komponen React secara reaktif, dinamis, dan terstruktur).",
      { lineGap: 2 }
    );

    // --- PAGE 4: DETAIL PENJELASAN PER BAHASA ---
    doc.addPage();
    currentPage++;
    addHeader("3. DETAIL BAHASA & DOKUMENTASI");
    addFooter(currentPage);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(18).text("3. Dokumentasi Teknis Per Bahasa", 50, 70);
    doc.moveDown(1);

    // Section: JavaScript / Node.js
    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(13).text("A. JavaScript & Node.js");
    doc.moveDown(0.5);
    doc.fillColor(textColor).font("Helvetica").fontSize(10).text(
      "JavaScript adalah mesin penggerak interaktivitas di web. Di sisi client, JS mengontrol animasi, penyerahan formulir (AJAX), dan validasi instan. Di sisi server, Node.js mengeksekusi mesin JavaScript V8 untuk menangani ribuan koneksi konkuren dengan non-blocking I/O.\n\n" +
      "Fitur JavaScript ES6+ yang banyak digunakan di aplikasi ini:\n" +
      "\u2022 Promises & Async/Await: Untuk melakukan panggilan API eksternal tanpa mengunci antarmuka UI.\n" +
      "\u2022 Arrow Functions: Penulisan fungsi yang lebih ringkas dan modern.\n" +
      "\u2022 LocalStorage API: Digunakan di js/helpers.js untuk menyimpan sesi token pengguna secara lokal agar tidak perlu masuk berulang kali saat melakukan refresh halaman.",
      { align: "justify", lineGap: 3 }
    );
    doc.moveDown(1.5);

    // Section: HTML5 & CSS3 (Tailwind CSS)
    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(13).text("B. HTML5 & CSS3 (Tailwind CSS)");
    doc.moveDown(0.5);
    doc.fillColor(textColor).text(
      "HTML5 menyediakan kerangka logis semantik yang ramah terhadap mesin pencari (SEO) dan teknologi aksesibilitas (seperti screen reader). Tag seperti <header>, <nav>, <section>, <main>, dan <footer> digunakan untuk membagi halaman secara rapi.\n\n" +
      "Tailwind CSS adalah framework CSS utility-first yang mempercepat proses desain. Di SiPinjam UPT, styling dikerjakan langsung di dalam file HTML menggunakan utility-classes:\n" +
      "\u2022 Layout Fleksibel: Menggunakan 'flex', 'grid', 'grid-cols-1 md:grid-cols-4' untuk tata letak responsif.\n" +
      "\u2022 Desain Konsisten: Menggunakan custom color brand-600 (Indigo) dan netral-900 (Charcoal).\n" +
      "\u2022 Spacing & Tipografi: Menggunakan class 'py-20', 'px-6', 'font-display', dan 'tracking-tight'.",
      { align: "justify", lineGap: 3 }
    );

    // --- PAGE 5: KESIMPULAN & TANYA JAWAB ---
    doc.addPage();
    currentPage++;
    addHeader("4. KESIMPULAN & ALUR KERJA");
    addFooter(currentPage);

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(18).text("4. Ringkasan & Alur Aliran Data", 50, 70);
    doc.moveDown(1);

    doc.fillColor(textColor).font("Helvetica").fontSize(10).text(
      "Sebagai penutup, berikut adalah skema ringkas bagaimana seluruh bahasa dan teknologi ini berkolaborasi membentuk ekosistem aplikasi SiPinjam UPT:",
      { lineGap: 3 }
    );
    doc.moveDown(1);
    ...
    // Diagram Box
    doc.save();
    doc.rect(50, doc.y, 495, 180).fill("#faf5ff").stroke("#d8b4fe");
    doc.fillColor("#581c87").font("Helvetica-Bold").fontSize(11).text("ALUR ALIRAN DATA APLIKASI:", 65, doc.y + 12);
    doc.font("Helvetica").fontSize(9.5).fillColor("#1f2937");
    doc.text(
      "1. Pengguna membuka Beranda (landing.html) yang disusun oleh HTML5 & CSS3 (Tailwind CSS).\n" +
      "2. Pengguna menekan tombol 'Cari & Pinjam', lalu masuk (login.html). Logika input ditangkap oleh JavaScript (js/helpers.js).\n" +
      "3. Permintaan masuk dikirim ke server.ts (TypeScript / Node.js) melalui endpoint asinkron POST /api/auth/login.php.\n" +
      "4. Server memverifikasi kecocokan password_hash di database state lokal (db_state.json).\n" +
      "5. Jika sukses, token sesi disimpan di LocalStorage pengguna melalui JavaScript.\n" +
      "6. Admin mengelola perlengkapan (admin/barang.html) dan dapat mengunggah foto barang secara langsung. Foto diupload ke /api/barang/upload_foto.php, diproses oleh middleware Multer, disimpan di uploads/barang/, dan jalurnya disimpan dalam db_state.json.",
      65, doc.y + 30, { lineGap: 4.5 }
    );
    doc.restore();

    doc.y = doc.y + 200; // Skip below the box

    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(12).text("Dukungan Teknis & Pengembangan:");
    doc.moveDown(0.5);
    doc.fillColor(textColor).font("Helvetica").fontSize(10).text(
      "Aplikasi ini dikonfigurasi untuk siap dikembangkan lebih jauh (scalable). Dengan fondasi TypeScript yang kuat pada backend, integrasi ke basis data relasional sesungguhnya (seperti PostgreSQL atau MySQL) maupun database Cloud (seperti Firebase Firestore) dapat dilakukan dengan perubahan kode minimal tanpa mengubah arsitektur inti sistem.",
    ...
    console.error("Gagal mengenerate PDF dokumentasi:", error);
    res.status(500).json({ success: false, error: "Gagal mengenerate PDF dokumentasi." });
  }
});
